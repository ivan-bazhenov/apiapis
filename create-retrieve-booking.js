...
var behaviour = function (credentials) {
	argsPost = {
	  data:
	  ...
	  headers: { "Content-Type": "application/xml", "Authorization": credentials }
	};
...
fs.exists('auth_key', function(exists) {
  if (exists) {
	behaviour(fs.readFileSync('auth_key', 'utf8'));
  } else {
    console.log('"auth_key" file is missing');
  }
});